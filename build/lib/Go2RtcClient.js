...
class Go2RtcClient {
    ...
    async isAvailable(timeoutMs) {
        try {
            await axios_1.default.get(`${this.url}/api/streams`, { timeout: timeoutMs || 2000 });
            return true;
        }
        catch {
            return false;
    ...
    async ensureStream(name, source) {
        ...
        await axios_1.default.put(`${this.url}/api/streams?${query.toString()}`, null, { timeout: 5000 });
    ...
    async getSnapshot(name, timeoutMs) {
        const response = await axios_1.default.get(`${this.url}/api/frame.jpeg`, {
            params: { src: name },
     ...
     */
    async openMjpegStream(name) {
        const response = await axios_1.default.get(`${this.url}/api/stream.mjpeg`, {
            params: { src: name },
            responseType: 'stream',
